...
##### Clone or rescue a block device
   ...
   dd rescue -v /dev/sda /dev/sdb logfile.log
...
##### duckduckgo search to w3m browser
   ...
   dd g(){ search=""; bang=""; for term in $@; do if [[ "$term" =~ -([A-Za-z0-9._%+-]*) ]]; then bang="\!${BASH_REMATCH[1]}" ; else search="$search%20$term" ; fi ; done ; w3m "https://www.duckduckgo.com/?q=$bang$search" ;}
...
##### Commands to setup my new harddrive! #4 Step! Try to recover as much as possible
   ...
   dd rescue -r 1 /dev/old_disk /dev/new_disk rescued.log

##### #3 Step! FIrst Pass quickly!

   dd rescue -n /dev/old_disk /dev/new_disk rescued.log
...
##### Open search engines from terminal
   ...
   dd g(){ ARGS="$@"; xdg-open "https://www.duckduckgo.com/?q=${ARGS}"; }
